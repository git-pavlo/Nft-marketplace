...
export default function NFTCard({ nft, reload, mode }) {
  ...
  if (!meta) {
    return (
      <div className="bg-slate-800 p-4 rounded-xl animate-pulse">
  ...
  return (
      ...
      <motion.div
        ...
        whileTap={{ scale: 0.97 }}
        className="bg-slate-800 p-4 rounded-xl shadow-xl relative hover:shadow-indigo-500/40 hover:border hover:border-indigo-500 transition-all"
      >
        {/* {nft.category && (
          <motion.div
            initial={{ opacity: 0, y: -6 }}
            animate={{ opacity: 1, y: 0 }}
            className="absolute bottom-20 right-2 bg-indigo-600/90 text-white text-xs px-3 py-1 rounded-full shadow"
          >
            {nft.category}
          </motion.div>
        )}       */}
        ...
        >
          <div className="mb-3 w-full aspect-square overflow-hidden rounded-xl bg-slate-800">
            <img
              src={meta.image}
          ...
          isOwner ? (
            <>
              <input
                ...
                className="mt-2 p-2 rounded bg-slate-700 text-white"
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className={`mt-2 w-full py-2 rounded font-bold
                  ${canceling
                    ? "bg-gray-600"
        ...
        {mode === "wallet" && (
          <>
            ...
            <button
              ...
              className="w-full mt-3 bg-indigo-600 py-2 rounded hover:bg-indigo-700"
            >
              List for Sale
            </button>
          </>
